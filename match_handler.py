...
class RegistrationHandler(GenericMessageHandler):
    ...
    async def message_teams(self, message, permission):
        ...
        team1 = [player.name for player in self.match_next.teams[0]]
        team2 = [player.name for player in self.match_next.teams[1]]
        await self.reply(message, f"Team 1:{team1}\nTeam 2:{team2}")
    ...
    async def message_rank(self, message, permission):
        ...
        if not res:
            ...
            await message.author.send(f"Ranklist:\n{self.list_ranks()}")
            return
        ...
        if not message.author.id in self.player_pool:
            ...
            player.set_rank(rank)
        await message.author.send(f"Your registrated rank:{ranks[rank]}")
    ...
    async def message_maps(self, message, permission):
        ...
        if len(res) != 7:
            ...
            # await self.reply(message,f"!maps {self.list_maps()}")
            return
        player = self.player_pool[message.author.id]
        try:
